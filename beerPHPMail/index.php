...
try {
    ...
    for ($i = 1; $i < 4; $i++) {
        ...
        $main = str_replace('<div style="display: none;">{beer}</div>', file_get_contents('templates/beer.html'), $main);

        $stmt = $pdo->prepare('SELECT * FROM site.beer WHERE id = :id');
        $stmt->bindParam(':id', $i, PDO::PARAM_INT);
        $stmt->execute();
        $row = $stmt->fetch(PDO::FETCH_ASSOC);
        $main = str_replace('{link}',$row['link'], $main);
        $main = str_replace('{text}',$row['name'], $main);
        $main = str_replace('{img}',$row['imgL'] , $main);
...
if (isset($_SESSION['user_id']))
    $footer = str_replace('<div style="display: none;">{Edit}</div>', file_get_contents('templates/cEdit.html'),$footer);
